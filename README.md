...
- **阅读代码**：回答中的常用语言代码块、历史文件差异和离线 HTML 支持语法高亮，未知语言保留纯文本。
- **查看改动**：工具卡直接展示增删行，右侧“变更”按会话列出 edit/write 的逐次修改。支持展开、分页和原始 patch；不包含 bash 或外部编辑器的未记录修改。
- **浏览文件**：顶部“文件与变更”打开项目文件树。单击选择、双击文件（或 Enter）交给本机默认程序打开；隐藏项可切换，外部文件变化点击刷新。
- **网页搜索**：默认启用 web_search，模型需要时调用 DeepSeek 服务端搜索，来源与部分结果状态可见；设置中可关闭。搜索有独立调用记录和用量，沿用本机 DeepSeek 凭证。
- **读取网页**：模型可在搜索后调用 web_fetch 核对原文，也可直接读取你提供的公开 HTTP(S) 链接。页面摘要回退、截断与来源会明确标注；完整已保存文本在工具详情中查看。抓取本身无需搜索凭证，关闭搜索后仍可用。未启用代理时直连；已配置环境代理或 macOS/Windows 系统 HTTP(S) 静态代理时自动使用，网站要求浏览器验证时会明确提示。
