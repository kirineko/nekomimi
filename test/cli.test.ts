...
it('CLI SIGINT ends a blocked HTTP request with a durable cancelled run', async () => {
  ...
  await new ConfigStore(join(dir,"home")).save("auth",{revision:0,apiKey:key});
  let arrived!: () => void;
  ...
  const child = spawn(process.execPath, [resolve('dist/cli.js'), 'run', 'synthetic', '--workspace', dir, '--home', join(dir,'home'), '--session', join(dir, 's'), '--base-url', `http://127.0.0.1:${address.port}`, '--json'], { env: { ...process.env, DEEPSEEK_API_KEY: key }, stdio: ['ignore', 'pipe', 'pipe'] });
